        ...
        <div>
          ...
          <main className="col-span-12 md:col-span-9 flex flex-col md:p-2 min-h-[88vh]">
            ...
            <div className="flex justify-between md:hidden px-6 mb-2 mt-6 border-t border-solid dark:border-none">
              <NavLink to={"../"}>
                <AiOutlineRight className='text-3xl p-1 bg-white rounded-lg dark:bg-gray-700 text-slate-600 dark:text-text-gray cursor-pointer'/>
              </NavLink>
              <span className="dark:text-text-gray">سبد خرید شما</span>
              <BiDotsVerticalRounded className='text-3xl p-1 bg-white dark:bg-gray-700 text-slate-700 dark:text-text-gray cursor-pointer rounded-lg'/>
            </div>
            {/* -----------------------------desktop size--------------------- */}
            <div className="w-full hidden md:flex justify-between  text-slate-700 pr-8 pl-28 mt-2 bg-white py-4 rounded-lg dark:text-text-gray dark:bg-gray-700">
             ...
             <div
            className={`w-full h-auto lg:max-h-[78vh] mt-2 bg-white p-1 rounded-lg grid grid-cols-12 lg:w-2/3 lg:overflow-scroll mb-40 lg:mb-0 scrollbar-hide ${
              !cart.length && "lg:w-full mb-0"
            } dark:bg-main-gray`}
          >
            {cart.length ? (
              cart.map((item) => {
                ...
                return (
                  <>
                    {loading ? (
                      <div
                        className="col-span-12 grid grid-cols-12 gap-1 py-4 border-b-2 border-solid "
                        key={item.id}
                      >
                        {/* -------------img--------------------------- */}
                        ...
                        <div className="col-span-8 p-2 bg-white pt-6 dark:text-gray-200 dark:bg-main-gray">
                          ...
                          <div>
                            <div className="flex items-center gap-2 mt-6 text-sm text-slate-500 dark:text-text-gray">
                              <div
                                className={`w-5 h-5 ${item.selectedColor.val} rounded-full dark:border`}
                              ></div>
                              <span className="dark:text-text-gray ">
                        ...
                        <div className="col-span-12 p-2 bg-white flex items-center mr-8 dark:bg-main-gray">
                          <div className="w-24 flex justify-between items-center p-2 border-2 border-solid border-slate-300 rounded-lg text-orange-500 dark:text-text-gray ">
                            <button onClick={() => handleInc(item)}>
                              <AiOutlinePlus />
                            </button>
                            <span>{item.quantity}</span>
                            <button onClick={() => handleDec(item)}>
                              {item.quantity > 1 ? (
                                <AiOutlineMinus />
                              ) : (
                                <FaRegTrashAlt className="text-red-500 dark:text-red-300" />
                              )}
                            </button>
                          </div>
                          <div className="mr-6 text-slate-500 dark:text-yellow-300 text-xl">
              ...
              <div className={`fixed lg:static bottom-0 lg:flex h-auto lg:h-96 w-full lg:w-1/3 bg-white dark:bg-gray-700 mt-8 border-2  border-solid rounded-lg flex-col justify-between  dark:border-slate-600 p-4 ${!cart.length && "hidden"}`}>
                ...
                <div className="w-full p-4">
                  {loading ?(
                    <span className="text-2xl text-rose-500 dark:text-slate-200">
                      {total.toLocaleString("fa-IR")} تومان
                    </span>
                  ):(
                    <div className="w-60 h-10 bg-slate-300 rounded-lg animate-pulse"></div>
                  )}
                </div>

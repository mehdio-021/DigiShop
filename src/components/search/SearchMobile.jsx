...
const SearchMobile = () => {
  ...
  const handleSearchWindow = (e) => {
    const filteredItems=data.products.filter((item)=>item.name.includes(e.target.value))
      ...
      {searchWindow ? (
        <div>
          <div className="w-[100vw] h-20 bg-slate-300 dark:bg-main-gray absolute top-0 right-0 z-[80] flex justify-center items-center px-8">
            ...
            />
            <GiCrossMark
              className="text-3xl mr-6 text-slate-300 cursor-pointer"
              onClick={() => setSearchWindow(false)}
            />
            <BiSearchAlt className="absolute right-12 text-2xl text-slate-400 text-slate-200" />
          </div>
          <div className="w-[100vw] h-screen bg-gray-700 absolute top-[5.5rem] right-0 z-[80] fadeShadow flex py-6 ">
            {filteredProducts ? (

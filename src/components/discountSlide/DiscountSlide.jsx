  ...
  return (
    ...
    >
      ...
      {filteredProducts.map((product)=>(
        ...
        className="w-full h-full border border-solid border-red-500 dark:border-slate-400 cursor-pointer hover:shadow-xl hover:shadow-red-500 dark:shadow-slate-200 hover:transition-all hover:duration-300 dark:rounded-lg"
        >
        ...
        <div className="flex justify-between items-center flex-col h-full py-2 pb-6 px-4 text-sm md:text-base  dark:bg-gray-700">
            <img src={product.image} alt={product.name}  className="w-36 h-36 bg-white object-cover dark:bg-gray-700"/>
            <span className="text-[10px] md:text-xs text-slate-600 dark:text-text-gray">{product.name}</span>
            <div className="flex justify-between w-full mt-4">
                <span className="w-8 h-8 flex justify-center items-center text-xs md:text-sm text-white bg-rose-500 rounded-full">
                    {product.discount}
                </span>
                <span className="block text-slate-600 dark:text-yellow-300 text-xs md:text-base mt-1">
                    {product.price.toLocaleString("fa-IR") } {" "}تومان
                </span>
            </div>
            <span className=" w-full flex justify-end text-slate-400 line-through text-xs">
